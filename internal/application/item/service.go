...
func (s *Service) Create(ctx context.Context, in CreateInput) (*domainitem.Item, error) {
	...
	if name == "" {
		return nil, domainerr.InvalidArgument("name is required", map[string]any{"field": "name"})
	}
...
func (s *Service) GetByID(ctx context.Context, id domainitem.ID) (*domainitem.Item, error) {
	if id.IsZero() {
		return nil, domainerr.InvalidArgument("id is required", map[string]any{"field": "id"})
	}
...
func (s *Service) Delete(ctx context.Context, id domainitem.ID) error {
	if id.IsZero() {
		return domainerr.InvalidArgument("id is required", map[string]any{"field": "id"})
	}
